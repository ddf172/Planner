#include "algorithm/AlgorithmManager.hpp"
#include <filesystem>
#include <iostream>
#include <fstream>
...
AlgorithmManager::AlgorithmManager(const std::string& algorithmDirectory) 
    : algorithmsDirectory(algorithmDirectory) {
    scanAlgorithms();
}

bool AlgorithmManager::scanAlgorithms(const std::string& algorithmDirectory) {
...
std::vector<AlgorithmInfo> AlgorithmManager::getAlgorithms() const {
...
std::vector<std::string> AlgorithmManager::getAlgorithmNames() const {
...
AlgorithmInfo AlgorithmManager::getAlgorithm(const std::string& name) const {
...
bool AlgorithmManager::hasAlgorithm(const std::string& name) const {
    return algorithms.find(name) != algorithms.end();
}

std::vector<std::string> AlgorithmManager::validateConfiguration(const std::string& algorithmName, const json& config) const {
...
std::string AlgorithmManager::getAlgorithmPath(const std::string& name) const {
    auto it = algorithms.find(name);
    if (it != algorithms.end()) {
...
void AlgorithmManager::loadAlgorithmFromDirectory(const std::string& algorithmDir) {
...
bool AlgorithmManager::isValidAlgorithmDirectory(const std::string& path) const {
